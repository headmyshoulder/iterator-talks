...
\frame{
  ...
  \begin{itemize}
   \item Ranges are based on iterators
   \item New concepts:
   \begin{itemize}
     \item Iterable -- Container, holding the elements 
     \item Range -- Lightweight adapters (decorators)
   \end{itemize}
   \item Three kinds of ranges
   \begin{itemize}
    \item Iterator pairs, Iterator and a count, Iterator and a 
predicate
   \end{itemize}
...
\begin{frame}[fragile]
 ...
 \vspace{2ex}
 Example: Sampled function $f(x)$
 \begin{align*}
  t_n & = & 0.1 n & \quad \quad & \textrm{(Sampling)} & \quad 
\quad \quad \quad \quad \\
  x_n & = & \sin( 2 t_n ) + 0.1 & & \textrm{(Numerical sequence -- sampled 
function)} &
 \end{align*}
...
auto seq3 = join( seq1 , seq2 );
  ...
  \vspace{2ex}
  Cartesian sequences -- missing yet
...
\frame{
  ...
  \begin{itemize}
    ...
    \item More complicated break condition,\\e.g. count and predicate
  \end{itemize}
